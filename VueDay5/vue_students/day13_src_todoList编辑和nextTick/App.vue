...
export default {
  ...
  created() {
    ...
    this.$set(this.hobbies,"isEtid",false);
  },
  watch:{
    hobbies:{
      deep:true,
  ...
  methods:{
    ...
    openUpdate(v){
      this.hobbies[v].isEtid=true;

    },
    closeUpdate(v){
      this.hobbies[v].isEtid=false;
    },
    deletedate(v){
  ...
  mounted() {
    ...
    this.$bus.$on('opentext',this.openUpdate);
    this.$bus.$on('closeupdate',this.closeUpdate);
    this.subId=pubsub.subscribe('addvalue',this.adddate);
  },
  beforeDestroy() {
